...
import { orders, orderItems, products, productVariants } from "@jetbeans/db/schema"
import { sql, and, gte, lte, eq, sum, count } from "@jetbeans/db/drizzle"
...
export async function getCartAbandonment(range: DateRange): Promise<{ rate: number; abandonedCarts: number; completedCarts: number }> {
  ...
  const { analyticsEvents } = await import("@jetbeans/db/schema")
  const { countDistinct } = await import("@jetbeans/db/drizzle")
