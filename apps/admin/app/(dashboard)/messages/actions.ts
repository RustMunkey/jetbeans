...
export async function getAllMessagesForList() {
	const workspace = await requireWorkspace()
	const session = await auth.api.getSession({ headers: await headers() })
	if (!session) throw new Error("Unauthorized")
	...
	// Fetch team messages and inbox emails in parallel
	const [teamMsgs, emails] = await Promise.all([
		db
			.select({
				id: teamMessages.id,
				senderId: teamMessages.senderId,
	...
	for (const msg of teamMsgs) {
		items.push({
			...
			subject: msg.channel ? `#${msg.channel}` : "",
			preview: (msg.body || "").replace(/<[^>]*>/g, "").slice(0, 100),
