...
Official TypeScript SDK for the JetBeans Storefront API.

## Installation

```bash
npm install @jetbeans/sdk
# or
pnpm add @jetbeans/sdk
# or
yarn add @jetbeans/sdk
# or
bun add @jetbeans/sdk
```
...
const client = new JetBeans({
  apiKey: "sf_your_api_key", // Get from JetBeans Admin > Settings > Storefronts
})
...
```typescript
import { JetBeans, JetBeansError } from "@jetbeans/sdk"

try {
  const { product } = await client.products.get("nonexistent")
} catch (error) {
  if (error instanceof JetBeansError) {
...
const client = new JetBeans({
  apiKey: "sf_...",                              // Required
  baseUrl: "https://app.jetbeans.cafe",          // Optional, defaults shown
...
export const jetbeans = new JetBeans({
...
import { jetbeans } from "@/lib/jetbeans"

export default async function ProductsPage() {
  const { products } = await jetbeans.products.list({ limit: 20 })
