...
const DEFAULT_BASE_URL = "https://app.jetbeans.cafe"
const DEFAULT_TIMEOUT = 30000

/**
 * JetBeans Storefront SDK Client
 *
 * @example
 * ```ts
...
export class JetBeans {
  ...
  constructor(config: JetBeansConfig) {
    if (!config.apiKey) {
      throw new Error("JetBeans SDK requires an apiKey")
    }
  ...
  async fetch<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new JetBeansError(errorMessage, response.status, errorCode)
      }
      ...
      if (error instanceof Error && error.name === "AbortError") {
        throw new JetBeansError("Request timed out", 408, "TIMEOUT")
      }

      throw new JetBeansError(
...
class ProductsResource {
  constructor(private client: JetBeans) {}
