import { pgTable, text, boolean, timestamp, jsonb, uuid, unique, integer } from "drizzle-orm/pg-core"
...
export const outgoingWebhookEndpoints = pgTable("outgoing_webhook_endpoints", {
	id: uuid("id").primaryKey().defaultRandom(),
	name: text("name").notNull(),
...
export const incomingWebhookUrls = pgTable("incoming_webhook_urls", {
	id: uuid("id").primaryKey().defaultRandom(),
